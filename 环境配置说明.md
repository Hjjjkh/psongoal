# 环境配置说明

## 📋 概述

本文档说明 Monorepo 结构下的环境变量配置，确保本地开发和 Zeabur 部署正常。

---

## 🏗️ 项目结构

```
psongoal/
├── packages/
│   └── web/              # 网页版（现有项目）
│       ├── .env.local    # 本地环境变量（不提交到 Git）
│       └── ...
├── package.json          # 根 package.json（workspace 配置）
└── pnpm-workspace.yaml   # pnpm workspace 配置
```

---

## 🔧 本地开发环境配置

### 1. 创建环境变量文件

在 `packages/web/` 目录下创建 `.env.local` 文件：

```env
# Supabase 配置
NEXT_PUBLIC_SUPABASE_URL=https://your-project.supabase.co
NEXT_PUBLIC_SUPABASE_ANON_KEY=your-anon-key-here
```

### 2. 获取 Supabase 配置

1. 登录 [Supabase Dashboard](https://app.supabase.com)
2. 选择你的项目
3. 进入 **Settings** > **API**
4. 复制：
   - **Project URL** → `NEXT_PUBLIC_SUPABASE_URL`
   - **anon public** key → `NEXT_PUBLIC_SUPABASE_ANON_KEY`

### 3. 验证配置

```bash
# 从根目录运行
pnpm dev:web

# 或从 packages/web 目录运行
cd packages/web
pnpm dev
```

访问 `http://localhost:3000`，确认应用正常运行。

---

## 🚀 Zeabur 部署配置

### 1. 连接 GitHub 仓库

1. 登录 [Zeabur Dashboard](https://zeabur.com)
2. 点击 **New Project**
3. 选择 **Import from GitHub**
4. 选择你的仓库

### 2. 配置项目设置

#### 根目录设置

在 Zeabur 项目设置中，设置：
- **Root Directory**: `packages/web`

#### 环境变量设置

在 Zeabur 项目设置中，添加以下环境变量：

```
NEXT_PUBLIC_SUPABASE_URL=https://your-project.supabase.co
NEXT_PUBLIC_SUPABASE_ANON_KEY=your-anon-key-here
```

### 3. 构建配置

Zeabur 会自动检测 Next.js 项目，使用以下构建命令：

```bash
# Zeabur 会自动运行
cd packages/web
pnpm install
pnpm build
```

### 4. 部署验证

1. 等待构建完成
2. 访问 Zeabur 提供的 URL
3. 确认应用正常运行

---

## 📝 环境变量说明

### `NEXT_PUBLIC_SUPABASE_URL`

- **类型**: 字符串
- **说明**: Supabase 项目 URL
- **格式**: `https://xxxxx.supabase.co`
- **必需**: ✅ 是

### `NEXT_PUBLIC_SUPABASE_ANON_KEY`

- **类型**: 字符串
- **说明**: Supabase 匿名公钥（用于客户端访问）
- **格式**: `eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...`
- **必需**: ✅ 是

---

## 🔒 安全注意事项

1. **不要提交 `.env.local` 到 Git**
   - 已配置在 `.gitignore` 中
   - 包含敏感信息

2. **使用环境变量管理**
   - 本地开发：使用 `.env.local`
   - 生产环境：使用 Zeabur 环境变量配置

3. **密钥轮换**
   - 定期检查 Supabase 密钥
   - 如果泄露，立即在 Supabase Dashboard 中重置

---

## 🐛 常见问题

### 1. 本地开发无法连接 Supabase

**问题**: 应用无法连接到 Supabase

**解决**:
1. 检查 `.env.local` 文件是否存在
2. 检查环境变量是否正确
3. 检查 Supabase 项目是否正常运行
4. 重启开发服务器

### 2. Zeabur 部署失败

**问题**: Zeabur 构建失败

**解决**:
1. 检查根目录设置是否为 `packages/web`
2. 检查环境变量是否配置
3. 查看 Zeabur 构建日志
4. 确认 `package.json` 中的脚本正确

### 3. 环境变量未生效

**问题**: 环境变量配置了但未生效

**解决**:
1. 确认变量名以 `NEXT_PUBLIC_` 开头（客户端变量）
2. 重启开发服务器
3. 清除 `.next` 缓存：`rm -rf .next`
4. 重新构建：`pnpm build`

---

## 📚 相关文档

- [Next.js 环境变量文档](https://nextjs.org/docs/basic-features/environment-variables)
- [Supabase 文档](https://supabase.com/docs)
- [Zeabur 文档](https://zeabur.com/docs)

---

**最后更新**: 2024-12-20

