 ...
 * 
 * 设计原则：
 * 1. This API is a system-level adjudicator for marking actions incomplete.
 * 2. completed_at is the single source of truth.
 * 3. Frontend state is not trusted. All decisions happen on the server.
 * 4. 已完成的 Action（completed_at IS NOT NULL）永远不可撤销
 * 5. 只有未完成的 Action 才允许记录未完成
 * 6. 所有判断逻辑在后端完成，前端不能绕过
 * 
 * 为什么 API 层要再次校验 completed_at：
 * - 这是 API 的裁决边界，确保已完成 Action 永远不可撤销
